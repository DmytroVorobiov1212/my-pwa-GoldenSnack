  ...
  const handleSubmit = async event => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/device-faults`, {
        ...
        body: JSON.stringify({ faultType, note: note.trim() }),
      });
      const result = await response.json();
      if (!response.ok) {
        if (response.status === 401) forgetDevice();
  ...
  return (
    <section className={css.container}>
      <div className={css.topRow}>
        ...
        <span className={css.sectionLabel}>PORUCHA · {device.machineName.toUpperCase()}</span>
      </div>
      <div className={css.heading}>
        <h1>Nahlásit poruchu</h1>
        <p>Vyberte zařízení. Balička <strong>{device.machineName}</strong> se doplní automaticky.</p>
      </div>
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className={css.submitButton} disabled={isSubmitting || !faultType}>
